		...
		return false;
          ...
          <form id="adbranch" name="adbranch" method="post" action="" onsubmit="return validation()">
            ...
            <table width="678" border="1">
              ...
              <?php
			  $resultbranch1 = mysql_query("SELECT DISTINCT prod_id FROM  orders ");   

			 	//	image	color	cost	quantity						
				while($row1 = mysql_fetch_array($resultbranch1))
				{
					 
					 $resultbranch = mysql_query("SELECT * FROM  products where prod_id='$row1[prod_id]'");
					 $row = mysql_fetch_array($resultbranch);
					   
					   $resultbranch = mysql_query("SELECT * FROM  orders where prod_id='$row1[prod_id]'");
					 $pqty = mysql_num_rows($resultbranch);
